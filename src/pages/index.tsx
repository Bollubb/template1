...
export default function Home(): JSX.Element {
  ...
  useEffect(() => {
    ...
    try {
      // favorites
      const rawFavsInit = localStorage.getItem(LS.favorites);
      if (rawFavsInit) {
        const arr = JSON.parse(rawFavsInit) as string[];
        setFavoriteIds(new Set(arr));
      }

      // read ids
      const rawReadInit = localStorage.getItem(LS.read);
      if (rawReadInit) {
        const arr = JSON.parse(rawReadInit) as string[];
