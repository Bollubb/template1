...
export default function UtilityHub({ onBack }: { onBack: () => void }) {
  ...
  // ===== PATCH 5 – Intelligent favorites (usage-based) =====
  const LS_USAGE = "nd_interactions_usage_v1";
  const LS_PAIR_USAGE = "nd_interactions_pair_usage_v1";

  const readUsage = (): Record<string, number> => safeJson<Record<string, number>>(isBrowser() ? localStorage.getItem(LS_USAGE) : null, {});
  const writeUsage = (v: Record<string, number>) => { if (isBrowser()) localStorage.setItem(LS_USAGE, JSON.stringify(v)); };

  const readPairUsage = (): Record<string, number> => safeJson<Record<string, number>>(isBrowser() ? localStorage.getItem(LS_PAIR_USAGE) : null, {});
  const writePairUsage = (v: Record<string, number>) => { if (isBrowser()) localStorage.setItem(LS_PAIR_USAGE, JSON.stringify(v)); };

  const bumpDrug = (id: string) => {
    const u = readUsage();
    u[id] = (u[id] || 0) + 1;
    writeUsage(u);
  };

  const pairKey2 = (aId: string, bId: string) => (aId < bId ? `${aId}__${bId}` : `${bId}__${aId}`);
  const bumpPair = (aId: string, bId: string) => {
    const u = readPairUsage();
    const k = pairKey2(aId, bId);
    u[k] = (u[k] || 0) + 1;
    writePairUsage(u);
  };

  const getTopUsedIds = (limitN = 8) => {
    const u = readUsage();
    return Object.keys(u)
      .sort((x, y) => (u[y] || 0) - (u[x] || 0))
      .slice(0, limitN);
  };

  const getSuggestedSecondIds = (firstId: string, limitN = 6) => {
    const u = readPairUsage();
    ...
    for (const k of Object.keys(u)) {
      ...
      if (x === firstId) scores[y] = (scores[y] || 0) + (u[k] || 0);
      else if (y === firstId) scores[x] = (scores[x] || 0) + (u[k] || 0);
    }

    // boost favorites
    for (const p of favPairs) {
      if (p.a === firstId) scores[p.b] = (scores[p.b] || 0) + 50;
      if (p.b === firstId) scores[p.a] = (scores[p.a] || 0) + 50;
    }

    return Object.keys(scores)
      .sort((x, y) => (scores[y] || 0) - (scores[x] || 0))
      .slice(0, limitN);
  };
...
function ToolInteractions({ onSave, onUpsell }: { onSave: (item: UtilityHistoryItem) => void; onUpsell: (t: string, d: string) => void }) {
  ...
  const [favPairs, setFavPairs] = useState<FavPair[]>(() => {
    ...
    },
    {
      id: "budesonide",
      name: "Budesonide",
      group: "Corticosteroidi inalatori",
      interactions: [],
    },
    {
      id: "levetiracetam",
    ...
    {
      id: "insulina_glargine",
      name: "Insulina glargine",
      group: "Antidiabetici",
      interactions: [],
    },
    {
  ...
  function confirm() {
    ...
    // usage tracking (preferiti intelligenti)
    bumpDrug(a.id);
    bumpDrug(b.id);
    bumpPair(a.id, b.id);
          ...
          aboveList={
            q1.trim() === "" ? (
              <div style={{ marginTop: 10 }}>
                <div style={{ fontSize: 12, fontWeight: 900, opacity: 0.85, marginBottom: 8 }}>⭐ Più usati da te</div>
      ...
      {step === 2 && (
        <StepPick
          ...
          aboveList={
            a ? (
              <div style={{ marginTop: 10 }}>
                <div style={{ fontSize: 12, fontWeight: 900, opacity: 0.85, marginBottom: 8 }}>🧠 Suggeriti (uso reale + preferiti)</div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {getSuggestedSecondIds(a.id, 6)
                    .map((id) => byId.get(id))
                    .filter(Boolean)
                    .map((e: any) => (
                      <button
                        ...
                        className="nd-tile nd-press"
                        onClick={() => {
                          setB(e); // auto-selezione farmaco 2
                        }}
                        style={{
                          padding: "6px 10px",
                          borderRadius: 999,
                ...
                {/* Assistant clinico (filtri) */}
                <SmartAssistant firstDrug={a.name} premium={limit.premium} />
              </div>
            ) : null
          }
          onPick={(e) => { bumpDrug(e.id); setB(e); }}
          footer={
