...
function ToolInteractions({ onSave, onUpsell }: { onSave: (item: UtilityHistoryItem) => void; onUpsell: (t: string, d: string) => void }) {
  ...
  const DB: Entry[] = [
    ...
    { id: "cefazolina", name: "Cefazolina", group: "Cefalosporine", also: ["Cefazolina"], interactions: [] },
    ...
    { id: "amikacina", name: "Amikacina", group: "Aminoglicosidi", also: ["Amikacin"], interactions: [] },
    { id: "tobramicina", name: "Tobramicina", group: "Aminoglicosidi", also: ["Tobi"], interactions: [] },
    ...
    { id: "colchicina", name: "Colchicina", group: "Metabolismo", also: ["Colchicina"], interactions: [] },
...
;
  ...
  const writeFavPairs = (next: FavPair[]) => {
    ...
    if (!isBrowser()) return;
    try {
      localStorage.setItem(LS.interactionsPairs as any, JSON.stringify(next.slice(0, 80)));
    } catch {}
  };
  const pairKey = (x: string, y: string) => (x < y ? `${x}__${y}` : `${y}__${x}`);
  const hasPair = (x: string, y: string) => favPairs.some((p) => pairKey(p.a, p.b) === pairKey(x, y));
  const addPair = (x: string, y: string) => {
    const key = pairKey(x, y);
    const next = [{ a: x, b: y, ts: Date.now() }, ...favPairs.filter((p) => pairKey(p.a, p.b) !== key)].slice(0, 80);
    writeFavPairs(next);
  };
  const removePair = (x: string, y: string) => {
    const key = pairKey(x, y);
          ...
          {a && (
            <div id="nd-suggest-anchor" style={{ marginTop: 10 }}>
              <div style={{ marginBottom: 10, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
                ...
                <div style={{ fontSize: 12, opacity: 0.78 }}>1 tap = risultato</div>
              </div>
