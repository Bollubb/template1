...
function shiftTextColor(code: ShiftCode): string {
  if (code === "M") return "rgba(56,189,248,0.95)";
  if (code === "P") return "rgba(250,204,21,0.95)";
  if (code === "N") return "rgba(167,139,250,0.98)";
  if (code === "R") return "rgba(34,197,94,0.95)";
  if (code === "F") return "rgba(248,113,113,0.95)";
  return "rgba(255,255,255,0.85)";
}

function shiftLabelStyle(code: ShiftCode): React.CSSProperties {
  // Minimal label: just a letter + color, with a tiny tinted background for readability.
  if (!code) return {};
  return {
    display: "inline-flex",
    alignItems: "center",
          ...
          return (
            ...
            >
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                ...
                {/* keep the top-right clean (no overlap) */}
                <div style={{ width: 22, height: 18 }} />
              </div>

              <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end" }}>
                {code ? (
                  <span style={shiftLabelStyle(code)}>{code}</span>
                ) : (
                  <span style={{ fontSize: 12, fontWeight: 950, opacity: 0.25, lineHeight: 1 }}>+</span>
                )}
              </div>
            </button>
          );
